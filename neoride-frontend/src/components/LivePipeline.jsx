...
const STEPS_META = [
  ...
  { icon: '🔍', name: 'ECM Param ID',        desc: 'L-BFGS-B optimising R0, R1, C1',                  color: '#f59e0b' },
  { icon: '⚙️', name: 'ECM Simulation',      desc: 'Forward-simulating 1RC Thevenin model',           color: '#ef4444' },
...
function LogLine({ text, color = '#94a3b8' }) {
  return (
    <div style={{ fontSize: '0.74rem', color, lineHeight: 1.7, fontFamily: 'monospace' }}>
...
export default function LivePipeline() {
  ...
  const runPipeline = async () => {
    if (!file) return
    setStatus('running'); setProgress(3); setResult(null); setError('')
    setTermLog([]); setStepStates(STEPS_META.map(() => 'pending')); setStepDetails(Array(6).fill(''))
    ...
    try {
      ...
      setStatus('error')
      addLog('Connection refused. Start Flask: python api/app.py', '#ef4444')
    }
  }
    ...
    <section id="live" style={{ padding: '80px 0', minHeight: '100vh', background: '#050d1a' }}>
      <style>{`
        ...
        .lp-step-card {
          ...
          border:1px solid rgba(255,255,255,0.06); background:#070f1e;
          transition:all 0.35s cubic-bezier(.4,0,.2,1); position:relative; overflow:hidden;
        }
        .lp-step-card.running {
          border-color:rgba(0,212,255,0.45); background:rgba(0,212,255,0.05);
          box-shadow:0 0 24px rgba(0,212,255,0.12);
        }
        .lp-step-card.done { border-color:rgba(16,185,129,0.35); background:rgba(16,185,129,0.05); }
        .lp-step-card .scan-line {
          position:absolute; left:0; right:0; height:2px;
          background:linear-gradient(90deg,transparent,#00d4ff,transparent);
          animation:lp-scan 1.2s linear infinite; display:none;
        }
        .lp-step-card.running .scan-line { display:block; }

        .lp-upload-zone {
          border:2px dashed rgba(0,212,255,0.25); border-radius:20px;
          padding:56px 40px; text-align:center; cursor:pointer;
          transition:all 0.3s; background:#0a1628; position:relative; overflow:hidden;
        }
        .lp-upload-zone:hover, .lp-upload-zone.dragover {
          border-color:#00d4ff; background:rgba(0,212,255,0.04);
          box-shadow:0 0 40px rgba(0,212,255,0.08);
        }
        .lp-upload-zone.has-file { border-color:#10b981; background:rgba(16,185,129,0.04); }
        ...
        .lp-run-btn:hover { transform:translateY(-2px) scale(1.03); box-shadow:0 0 40px rgba(0,212,255,0.5),0 8px 24px rgba(0,0,0,0.4); }
        ...
        .lp-chart-tab {
          ...
          transition:all 0.22s; white-space:nowrap; border:none;
        }
      `}</style>
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 2rem' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 52, animation: 'lp-fadeup 0.6s ease' }}>
          <div style={{
            ...
            color: '#10b981', padding: '5px 16px', borderRadius: 24, fontSize: '0.75rem',
          ...
          <h2 style={{
            fontSize: 'clamp(1.8rem,3vw,2.6rem)', fontWeight: 800, marginBottom: 14,
            ...
            >
              ...
              {file ? (
                <div style={{ animation: 'lp-fadeup 0.3s ease' }}>
                  <div style={{ fontSize: '3.5rem', marginBottom: 14, filter: 'drop-shadow(0 0 12px #10b981)' }}>✅</div>
                  <div style={{ fontSize: '1.15rem', fontWeight: 800, color: '#10b981', marginBottom: 6 }}>{file.name}</div>
                  <div style={{ fontSize: '0.84rem', color: '#475569' }}>{(file.size / 1024).toFixed(1)} KB · Click to change</div>
                </div>
              ) : (
                <>
                  ...
                  <div style={{ display: 'flex', gap: 10, justifyContent: 'center', marginTop: 20, flexWrap: 'wrap' }}>
                    {['mode', 'voltage_load', 'current_load', 'time', 'mission_type'].map(col => (
                      <span key={col} style={{
                        background: 'rgba(0,212,255,0.06)', border: '1px solid rgba(0,212,255,0.18)',
                        color: '#00d4ff', padding: '3px 10px', borderRadius: 8,
        ...
        {(status === 'running' || status === 'done' || status === 'error') && (
          <div style={{ animation: 'lp-fadeup 0.4s ease' }}>
            ...
            <div style={{
              background: '#0a1628', border: '1px solid rgba(0,212,255,0.15)',
              borderRadius: 18, padding: 28, marginBottom: 24, boxShadow: '0 4px 32px rgba(0,0,0,0.3)',
            }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                  ...
                  <div>
                    <div style={{ fontWeight: 800, fontSize: '1rem',
                      ...
                      {status === 'running' ? 'Pipeline executing...' :
                       status === 'done'    ? `Pipeline complete — ${file?.name}` : 'Pipeline error'}
                    </div>
                    {file && status !== 'error' && (
              ...
              {/* Progress bar */}
              <div style={{ height: 6, background: 'rgba(255,255,255,0.05)', borderRadius: 10, overflow: 'hidden', marginBottom: 24 }}>
                <div style={{
                  height: '100%', borderRadius: 10, width: `${progress}%`, transition: 'width 0.5s ease',
                  background: status === 'done'  ? 'linear-gradient(90deg,#10b981,#00d4ff)' :
                               status === 'error' ? '#ef4444' : 'linear-gradient(90deg,#00d4ff,#7c3aed,#10b981)',
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <div ref={logRef} className="lp-terminal">
                ...
                {termLog.map(l => <LogLine key={l.id} text={l.text} color={l.color} />)}
                {status === 'running' && (
            ...
            {status === 'done' && m && c && (
              <div style={{ animation: 'lp-fadeup 0.5s ease' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: 14, marginBottom: 32 }}>
                  ...
                  <MetricBox label="R0"                  value={m.ecm.R0_mohm}                   unit=" mOhm" color="#00d4ff" delay={240} />
                  <MetricBox label="R1"                  value={m.ecm.R1_mohm}                   unit=" mOhm" color="#7c3aed" delay={300} />
                  <MetricBox label="C1"                  value={m.ecm.C1_F}                      unit=" F"   color="#10b981" delay={360} />
                  <MetricBox label="tau = R1*C1"         value={m.ecm.tau_s}                     unit=" s"   color="#f59e0b" delay={420} />
                ...
                <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
                  {[
                    ...
                    <button key={tab.id} className="lp-chart-tab" onClick={() => setActiveTab(tab.id)}
                      style={{
                        background: activeTab === tab.id ? `linear-gradient(135deg,${tab.color}25,${tab.color}10)` : 'rgba(255,255,255,0.03)',
                        color:  activeTab === tab.id ? tab.color : '#475569',
                        border: activeTab === tab.id ? `1px solid ${tab.color}50` : '1px solid rgba(255,255,255,0.07)',
                      }}>
                      {tab.label}
                    </button>
                  ))}
                  ...
                  <ChartCard title="OCV - SOC Polynomial Fit" badge={`RMSE: ${m.ocv.rmse_mv} mV`} badgeColor="#00d4ff">
                    <ResponsiveContainer width="100%" height={320}>
                      <LineChart data={c.ocv} margin={{ top: 4, right: 16, left: -10, bottom: 8 }}>
                        <CartesianGrid stroke={gridClr} /><XAxis dataKey="soc" tick={tickStyle} /><YAxis tick={tickStyle} domain={['auto', 'auto']} />
                        <Tooltip {...tooltipStyle} /><Legend wrapperStyle={{ fontSize: 11, color: '#94a3b8' }} />
                  ...
                  <ChartCard title="ECM Voltage Fit vs Measured" badge={`RMSE: ${m.ecm.rmse_mv} mV`} badgeColor="#10b981">
                    <ResponsiveContainer width="100%" height={320}>
                      <LineChart data={c.ecm} margin={{ top: 4, right: 16, left: -10, bottom: 8 }}>
                        <CartesianGrid stroke={gridClr} /><XAxis dataKey="time" tick={tickStyle} /><YAxis tick={tickStyle} domain={['auto', 'auto']} />
                        <Tooltip {...tooltipStyle} /><Legend wrapperStyle={{ fontSize: 11, color: '#94a3b8' }} />
                  ...
                  <ChartCard title="EKF SOC Estimation (init=50%)" badge={`RMSE: ${m.ekf.rmse_pct}%  Conv: ${m.ekf.conv_time_s}s`} badgeColor="#7c3aed">
                    <ResponsiveContainer width="100%" height={320}>
                      <LineChart data={c.ekf} margin={{ top: 4, right: 16, left: -10, bottom: 8 }}>
                        <CartesianGrid stroke={gridClr} /><XAxis dataKey="time" tick={tickStyle} /><YAxis tick={tickStyle} domain={[0, 105]} />
                        <Tooltip {...tooltipStyle} /><Legend wrapperStyle={{ fontSize: 11, color: '#94a3b8' }} />
                  ...
                  <ChartCard title="SOC Error Over Time" badge={`Final: ${m.ekf.final_error_pct}%`} badgeColor="#f59e0b">
                    <ResponsiveContainer width="100%" height={320}>
                      <LineChart data={c.error} margin={{ top: 4, right: 16, left: -10, bottom: 8 }}>
                        <CartesianGrid stroke={gridClr} /><XAxis dataKey="time" tick={tickStyle} /><YAxis tick={tickStyle} />
                        <Tooltip {...tooltipStyle} />
                        <ReferenceLine y={2}  stroke="rgba(255,255,255,0.12)" strokeDasharray="4 3" />
                        <ReferenceLine y={-2} stroke="rgba(255,255,255,0.12)" strokeDasharray="4 3" />
                  ...
                  <ChartCard title="EKF Multi-Start Robustness" badge="All 5 inits converge" badgeColor="#10b981">
                    ...
                    <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 20 }}>
                      {Object.entries(m.multi_start).map(([k, v], i) => (
                        <div key={k} style={{
                          flex: 1, minWidth: 100, background: '#070f1e',
                          border: `1px solid ${multiColors[i]}30`, borderRadius: 10, padding: '12px 10px', textAlign: 'center',
                        }}>
                          <div style={{ fontSize: '1.3rem', fontWeight: 800, color: multiColors[i] }}>{[10, 30, 50, 70, 90][i]}%</div>
                          <div style={{ fontSize: '0.68rem', color: '#10b981', marginTop: 3 }}>ok {v.conv_time_s ? `${v.conv_time_s}s` : '—'}</div>
                          <div style={{ fontSize: '0.65rem', color: '#475569', marginTop: 2 }}>err: {v.final_error_pct}%</div>
                        </div>
                      ))}
                    </div>
                  </ChartCard>
                ...
                {/* Summary banner */}
                <div style={{
                  marginTop: 28, borderRadius: 16, padding: '22px 28px',
