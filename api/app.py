...
Accepts a CSV upload and runs the full 6-step pipeline,
...
def run_pipeline():
    ...
    # Save to a temp file so get_bol_data can read it
    suffix = '.csv'
    ...
    try:
        ...
        # Capacity prediction (correct init)
        ...
        # ── Downsample time-series to ≤ 400 pts ───────────────────────────
        N = len(time_s)
        ds = max(1, N // 400)
        ...
        # Multi-start chart - pre-compute all 5 EKF runs once, then downsample
        multi_chart = []
        multi_ekf_runs = {}
        for init_soc in [0.1, 0.3, 0.5, 0.7, 0.9]:
