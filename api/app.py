...
def run_pipeline():
    ...
    try:
        ...
        # Compute Q_max via Coulomb integration
        ...
        steps_log[-1]['status']  = 'done'
        steps_log[-1]['details'] = (
            f'R0={R0*1000:.2f} mOhm  R1={R1*1000:.2f} mOhm  '
            f'C1={C1:.0f} F  tau={params["tau"]:.1f}s  '
        ...
        # Time-series charts (max 400 points)
        N  = len(time_s)
        ds = max(1, N // 400)
        idx = list(range(0, N, ds))

        ecm_chart = [
            {'time': int(time_s[i]), 'measured': round(float(voltage_cell[i]), 4), 'simulated': round(float(v_ecm[i]), 4)}
            for i in idx
        ]
        ekf_chart = [
            {'time': int(time_s[i]), 'reference': round(float(soc_ref[i]) * 100, 2), 'ekf': round(float(soc_ekf[i]) * 100, 2)}
            for i in idx
        ]
        error_chart = [
            {'time': int(time_s[i]), 'error': round(float(soc_error[i]), 3)}
            for i in idx
        ]
        multi_chart = []
        for i in idx:
